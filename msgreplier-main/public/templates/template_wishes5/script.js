...
function startWishing() {
  ...
  personName = name;
  personDOB  = dob || '';
  personAge  = dob ? calculateAge(dob) : 0;
...
function populatePage() {
  ...
  const urlSender = urlParams.get('sender_name') || urlParams.get('sender');
  urlOccasion = urlParams.get('occasion') || 'Birthday';
  ...
  if (musicId && MUSIC_URLS[musicId]) {
    const bgMusic = document.getElementById('bgMusic');
    if (bgMusic) {
      bgMusic.src = MUSIC_URLS[musicId];
    }
  }
  ...
  if (ageBadge) {
    if (urlOccasion === 'Anniversary') {
      ...
      ageBadge.textContent = `🎂 Turning ${personAge} Today!`;
    }
  }
  document.getElementById('heroName').textContent   = personName;
  document.getElementById('heroTagline').textContent = `Wishing the most magical ${urlOccasion.toLowerCase()} to someone truly extraordinary.`;
  ...
  const hbBday = document.querySelector('.hb-birthday');
  if (hbBday) hbBday.textContent = urlOccasion;

  const saved = localStorage.getItem('bdayNote_' + personName);
  document.getElementById('noteText').textContent = urlMessage || saved || getDefaultNote(personName);
  document.getElementById('noteSig').textContent  = urlSender ? `— With Love, ${urlSender} 💖` : `— With Love 💖`;
  ...
  if (photoUrlParam) {
    try {
      if (photoUrlParam.startsWith('[')) {
        photos = JSON.parse(photoUrlParam);
      } else {
        photos = [{ url: photoUrlParam, caption: 'Moments ✨' }];
      }
    } catch (e) {
      console.error(e);
      photos = [{ url: photoUrlParam, caption: 'Moments ✨' }];
    ...
    photos = galleryPhotos.map((url, i) => ({ url, caption: `Memory ${i + 1} ✨` }));
  }

  const validPhotos = photos.filter(p => p && p.url);
  const hasPhotos = isPreview || (validPhotos && validPhotos.length > 0);

  if (!hasPhotos) {
    const gallerySec = document.getElementById('gallery');
    if (gallerySec) gallerySec.remove();
    document.querySelectorAll('.nav-link[data-section="gallery"]').forEach(el => el.remove());
    document.querySelectorAll('#mobileMenu a[href="#gallery"]').forEach(el => el.remove());
    const viewGalleryBtn = document.querySelector('.btn-outline[onclick="openGalleryShow()"]');
    if (viewGalleryBtn) viewGalleryBtn.remove();
  } else {
    const cards = document.querySelectorAll('.gif-scattered .gif-card');
    
    let displayPhotos = [];
    let isPlaceholderPreview = false;
    if (validPhotos.length > 0) {
      displayPhotos = validPhotos;
    } else if (isPreview) {
    ...
    displayPhotos.forEach((photo, idx) => {
      if (cards[idx]) {
        const img = cards[idx].querySelector('.gif-img');
        const caption = cards[idx].querySelector('.gif-caption');
        
        // Remove old placeholder if present
        const oldPlaceholder = cards[idx].querySelector('.example-picture-placeholder');
        if (oldPlaceholder) oldPlaceholder.remove();

        if (photo && (photo.url || isPlaceholderPreview)) {
          if (photo.url) {
            if (img) {
              img.src = photo.url;
              ...
              img.classList.toggle('fit-contain', !photoFitMode);
            }
          } else {
            if (img) img.style.display = 'none';
            const placeholderDiv = document.createElement('div');
            placeholderDiv.className = 'example-picture-placeholder';
            placeholderDiv.innerHTML = `
              <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100%; height: 160px; border: 2px dashed rgba(255,255,255,0.15); border-radius: 12px; color: rgba(255,255,255,0.4); font-family: sans-serif; font-size: 11px; text-transform: uppercase; font-weight: bold; letter-spacing: 1px; margin-bottom: 10px; background: rgba(255,255,255,0.02);">
                <span style="font-size: 20px; margin-bottom: 6px;">📸</span>
                <span>Example Picture</span>
              </div>
            `;
            cards[idx].insertBefore(placeholderDiv, cards[idx].firstChild);
          }
          if (caption) {
            caption.textContent = photo.caption || `Memory ${idx + 1}`;
          }
    ...
    for (let idx = displayPhotos.length; idx < cards.length; idx++) {
      if (cards[idx]) cards[idx].style.display = 'none';
    }

    galleryPhotos = displayPhotos.map(p => p.url);
  }

  const footerHearts = document.querySelector('.footer-hearts');
  if (footerHearts) {
    footerHearts.textContent = urlSender ? `💖 Made with love by ${urlSender} 💖` : `💖 Made with love 💖`;
  }
  ...
  document.title = `🎂 Happy ${urlOccasion}, ${personName}!`;

  const galleryTitle = document.querySelector('.gallery-section .section-title');
  if (galleryTitle) {
    galleryTitle.innerHTML = urlOccasion === 'Anniversary' ? 'Gallery' : 'Birthday <em>Gallery</em>';
  }

  const wishesTitle = document.querySelector('.wishes-section .section-title');
  if (wishesTitle) {
    wishesTitle.innerHTML = urlOccasion === 'Anniversary' ? 'Anniversary <em>Wishes</em>' : 'Birthday <em>Wishes</em>';
  }

  const fortuneTitle = document.querySelector('.fortune-section .section-title');
  if (fortuneTitle) {
    fortuneTitle.innerHTML = urlOccasion === 'Anniversary' ? 'Anniversary <em>Fortune</em>' : 'Birthday <em>Fortune</em>';
  }
...
function shareWA() {
  const url = document.getElementById('shareUrl')?.textContent || '';
  const msg = `🎂 Happy ${urlOccasion}, ${personName}! 🎉\n\nI made a special ${urlOccasion.toLowerCase()} page for you. Check it out here:\n${url}`;
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (urlName && (urlDOB || occasion === 'Anniversary')) {
    personName = urlName;
    personDOB  = urlDOB || '';
    personAge  = urlDOB ? calculateAge(urlDOB) : 0;
    urlOccasion = occasion;
    launchMainPage();
    return;
  }
...
function handlePhotoUploads(input) {
  ...
  const isFit = document.getElementById('photoFitToggle')?.classList.contains('active');
  ...
  files.forEach((file, i) => {
    ...
    reader.onload = function (e) {
      ...
      if (previews[i]) {
        ...
        // Apply fit mode
        previews[i].classList.toggle('fit-contain', !isFit);
      }
      loaded++;
...
// ─── PHOTO FIT TOGGLE ────────────────────────────────────
// photoFitMode: true = cover (fit to ratio), false = contain (fully visible)
// Read from URL param `fit_mode` if present (set by the create page)
const _fitModeParam = new URLSearchParams(window.location.search).get('fit_mode');
let photoFitMode = _fitModeParam === 'contain' ? false : true; // default ON

function togglePhotoFit() {
  photoFitMode = !photoFitMode;

  const btn   = document.getElementById('photoFitToggle');
  const label = document.getElementById('fitToggleText');
  const hint  = document.getElementById('photoFitHint');

  if (btn)   btn.classList.toggle('active', photoFitMode);
  if (btn)   btn.setAttribute('aria-pressed', String(photoFitMode));
  if (label) label.textContent = photoFitMode ? 'ON' : 'OFF';
  if (hint)  hint.textContent  = photoFitMode
    ? 'Photos will be cropped to fit the template frame.'
    : 'Full photos will be visible (may show letterboxing).';
...
function startFunStats(dobStr) {
  ...
  if (!dob) {
    ...
    dob = `${today.getFullYear() - 22}-${month}-${day}`;
  }
